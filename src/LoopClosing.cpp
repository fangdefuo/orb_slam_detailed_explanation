...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())//这三个if语句是判断闭环执行的条件
        {
...
{
    unique_lock<mutex> lock(mMutexLoopQueue);
    return(!mlpLoopKeyFrameQueue.empty());//队列非空，表明这个队列中有等待被选中成为候选帧的帧
}
...
{
    {
        // 从队列中取出一个关键帧
        unique_lock<mutex> lock(mMutexLoopQueue);
        mpCurrentKF = mlpLoopKeyFrameQueue.front();//如何从队列中取出一帧赋值给变量
        mlpLoopKeyFrameQueue.pop_front();//如何从队列中去掉一帧
        // Avoid that a keyframe can be erased while it is being process by this thread
        mpCurrentKF->SetNotErase();//当前帧刚被赋值不能被去除
    }
    ...
    for(size_t i=0; i<vpConnectedKeyFrames.size(); i++)
    {
        ...
        float score = mpORBVocabulary->score(CurrentBowVec, BowVec);//相似度得分应该小于１
    ...
    if(vpCandidateKFs.empty())//如果没有闭环
    {
        mpKeyFrameDB->add(mpCurrentKF);
    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
        ...
        // 将自己以及与自己相连的关键帧构成一个“子候选组”子候选组的定义
...
bool LoopClosing::ComputeSim3()
{
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nInitialCandidates; i++)
        {
            ...
            cv::Mat Scm  = pSolver->iterate(5,bNoMore,vbInliers,nInliers);//使用相似变换的原因是尺度不确定
