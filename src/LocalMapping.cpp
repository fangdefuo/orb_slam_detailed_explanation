...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        {
            // Safe area to stop
            while(isStopped() && !CheckFinish())//强制结束
            {
                // usleep(3000);
...
{
    unique_lock<mutex> lock(mMutexNewKFs);//插入关键帧时要加线程锁
    // 将关键帧插入到列表中
 ...
 * @return 如果存在，返回true
 */
bool LocalMapping::CheckNewKeyFrames()//非空返回１
{
    ...
    for(size_t i=0; i<vpMapPointMatches.size(); i++)
    {
        ...
        if(pMP)
        {
            if(!pMP->isBad())
            {
                ...
                if(!pMP->IsInKeyFrame(mpCurrentKeyFrame))//不是由当前帧生成的地图点
                {
                    // 添加观测
                    pMP->AddObservation(mpCurrentKeyFrame, i);//新匹配上的MapPoints和当前关键帧绑定
                    // 获得该点的平均观测方向和观测距离范围
                ...
                else // this can only happen for new stereo points inserted by the Tracking
                {
                    ...
                    mlpRecentAddedMapPoints.push_back(pMP);//不是最总确定的地图点
                }
            }
        }
    }    
...
void LocalMapping::MapPointCulling()
{
    ...
    list<MapPoint*>::iterator lit = mlpRecentAddedMapPoints.begin();//要检查那些地图点
    const unsigned long int nCurrentKFid = mpCurrentKeyFrame->mnId;

    int nThObs;//新定义的变量阈值，观测到关键点最小帧的个数
    if(mbMonocular)
        nThObs = 2;
    else
        nThObs = 3;
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        {
            // 邻接关键帧的场景深度中值
            const float medianDepthKF2 = pKF2->ComputeSceneMedianDepth(2);//就是景深
            // baseline与景深的比例
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            // 当前匹配在当前关键帧中的特征点
            const cv::KeyPoint &kp1 = mpCurrentKeyFrame->mvKeysUn[idx1];//特征点
            ...
            cv::Mat ray1 = Rwc1*xn1;//当前帧的位姿
            cv::Mat ray2 = Rwc2*xn2;//相邻关键帧的位姿
...
void LocalMapping::SearchInNeighbors()
{
    ...
    for(vector<KeyFrame*>::const_iterator vit=vpNeighKFs.begin(), vend=vpNeighKFs.end(); vit!=vend; vit++)
    {
        ...
        const vector<KeyFrame*> vpSecondNeighKFs = pKFi->GetBestCovisibilityKeyFrames(5);//与一级关键帧相连的关键帧是二级关键帧
...
cv::Mat LocalMapping::SkewSymmetricMatrix(const cv::Mat &v)//将向量转换成反对称矩阵
{
