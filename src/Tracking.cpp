 ...
 * Tracking 线程
 */
void Tracking::Track()// track包含两部分：估计运动、跟踪局部地图
{
    ...
    else// 步骤2：跟踪
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                if(mVelocity.empty() || mCurrentFrame.mnId<mnLastRelocFrameId+2)//这句表明刚刚进行过重定位，所以不能再次重定位，只能选择追踪参考帧
                {
                    // 将上一帧的位姿作为当前帧的初始位姿
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    bool bOKMM = false;//这里定义了新的变量
                    bool bOKReloc = false;
                    vector<MapPoint*> vpMPsMM;
                    vector<bool> vbOutMM;
                    cv::Mat TcwMM;

                    if(!mVelocity.empty())//mVelocity非空
                    {
        ...
        if(bOK)
        {
            ...
            if(!mLastFrame.mTcw.empty())
            {
                ...
                mVelocity = mCurrentFrame.mTcw*LastTwc; // 上一帧的相机到世界×当前帧的世界到相机？＝＝上一帧的相机到当前帧的相机Tcl
            }
            else
                mVelocity = cv::Mat();
            ...
            for(int i=0; i<mCurrentFrame.N; i++)
            {
                ...
                if(pMP)
                    ...
                    if(pMP->Observations()<1)//这个地图点没有帧观测到
                    {
    ...
    {
        // 计算相对姿态T_currentFrame_referenceKeyFrame　＝　＝　当前帧的世界到相机×参考帧的相机到世界
    ...
    if(mCurrentFrame.N>500)
    {
        ...
        for(int i=0; i<mCurrentFrame.N;i++)
        {
            ...
            if(z>0)
            {
                ...
                mCurrentFrame.mvpMapPoints[i]=pNewMP;//创建当前帧的地图点
            }
        }
...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        if(mpInitializer->Initialize(mCurrentFrame, mvIniMatches, Rcw, tcw, mvIniP3D, vbTriangulated))
        {
            ...
            mCurrentFrame.SetPose(Tcw);//计算得到当前帧的位姿
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // 评估关键帧场景深度，q=2表示中值
    float medianDepth = pKFini->ComputeSceneMedianDepth(2);//为什么要归一化？？？？是把z归一化到１吗？
    ...
    // x/z y/z 将z归一化到1 
    Tc2w.col(3).rowRange(0,3) = Tc2w.col(3).rowRange(0,3)*invMedianDepth;//将平移t归一化
    pKFcur->SetPose(Tc2w);
    ...
    for(size_t iMP=0; iMP<vpAllMapPoints.size(); iMP++)
    {
        if(vpAllMapPoints[iMP])
        {
            ...
            pMP->SetWorldPos(pMP->GetWorldPos()*invMedianDepth);//把地图点归一化
        }
    }

    // 这部分和SteroInitialization()相似
    //把当前帧的所有信息赋值给上一帧，继续获得当前帧的信息
    mpLocalMapper->InsertKeyFrame(pKFini);
...
void Tracking::CheckReplacedInLastFrame()
{
    for(int i =0; i<mLastFrame.N; i++)
    {
        ...
        if(pMP)
        {
            MapPoint* pRep = pMP->GetReplaced();//这步替换并没有看懂
            if(pRep)
            {
...
void Tracking::UpdateLastFrame()
{
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        if(bCreateNew)
        {
            // 这些生成MapPoints后并没有通过：//当生成地图点的时候都要为这些地图点添加属性
            // a.AddMapPoint、
            // b.AddObservation、
 ...
 */
void Tracking::UpdateLocalKeyFrames()//局部关键帧的构成
{
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            ...
            if(!pMP->isBad())
            {
                ...
                const map<KeyFrame*,size_t> observations = pMP->GetObservations();//得到能观测到当前地图点的关键帧
    ...
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
        ...
        // 策略2.2:自己的子关键帧
        const set<KeyFrame*> spChilds = pKF->GetChilds();//每个都有固定的函数去实现
...
bool Tracking::Relocalization()
{
    ...
    vpPnPsolvers.resize(nKFs);//根据候选帧构造pnp求解器

    vector<vector<MapPoint*> > vvpMapPointMatches;//匹配的地图点
